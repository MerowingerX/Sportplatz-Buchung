...
- `VALID_DURATIONS`-Konstante **entfällt**, ersetzt durch `MIN_DURATION_MINUTES` / `MAX_DURATION_MINUTES`

### 2. `web/routers/calendar.py` — Kalender-Router

#### 2a. `calendar_week()` (Zeile ~96–156)
- `num_slots = 360 // SLOT_MINUTES` — `SLOT_MINUTES` bleibt 15, Berechnung unverändert
- `start_hour`-Default von 16 auf 8 ändern
- Clamping `max(0, min(18, start_hour))` → auf `max(0, min(16, start_hour))` ändern (8+6=14, max 16 damit Fenster nicht über 22:00 hinausgeht)
- `prev_start_hour` / `next_start_hour`: Schrittweite von 2 beibehalten

#### 2b. Day-Router 
- Muss `time_slots` dynamisch generieren und ans Template übergeben (analog zu `calendar_week`)
- Gleiche `start_hour`-Logik wie die Wochenansicht
...
Der Day-Router (vermutlich `calendar_day()`) muss ebenfalls `time_slots` dynamisch generieren und ans Template übergeben. Aktuell tut er das nicht — das Template hartcodiert sie. Siehe Punkt 3.
...
| 5 | `web/routers/calendar.py` — start_hour=8, Clamping, Day-Router time_slots | mittel |
